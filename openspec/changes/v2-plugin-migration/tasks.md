...
- [ ] 1.1 Add `@opencode/plugin` and `@opencode/cli` as devDependencies; mark both `@opencode-ai/plugin` and `@opencode/plugin` optional peerDependencies (`peerDependenciesMeta`); verify `npm install` resolves cleanly from a clean `node_modules`
- [ ] 1.2 Update `package.json`: `main`/`"."` and `"./v1"` resolve to `plugin.v1.js`, `"./v2"` to `plugin.v2.js`; verify both subpaths resolve via dynamic import
