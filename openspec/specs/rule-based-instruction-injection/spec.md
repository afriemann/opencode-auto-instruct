...
**V2 event source and current limitation.** V2 has no `todo.updated`,
`message.updated`, or `tool.execute.after` events in its event vocabulary
(confirmed against the installed `@opencode/plugin` 2.0.4 type surface — no
todo domain exists at all). The nine todo-derived condition types
(`allTodosComplete`, `anyTodosComplete`, `noTodosInProgress`, `hasTodos`,
`todoListCreated`, `todoListCleared`, `firstTodoStarted`,
`allTodosCompleteOnce`, `todoCountAtLeast`) and the two tool-derived types
(`toolName`, `toolNameIn`) currently have no event source on V2 and SHALL
NOT match on that runtime; the plugin SHALL log one explicit warning per
affected rule at load time on V2, naming the rule and the reason, rather
than silently never firing. `messageFinished` SHALL be evaluated on V2
using the nearest equivalent event's finish-status field, and SHALL treat
an error/failure finish status as a non-match (V1's semantics test for a
successful/normal finish, not merely "any finish occurred").
Synthesizing the nine todo/tool-derived types from a different V2 event
(e.g. reconstructing todo state from a todo-management tool's completed
input) is a documented future enhancement, not a requirement of this
capability's current version — it depends on an unstable, untyped
assumption about a specific tool's shape.
...
- GIVEN a rule with a todo-derived or tool-derived condition type, running on V2
- WHEN the plugin loads
- THEN the plugin logs one warning naming the rule and stating the condition type has no V2 event source, and the rule never matches on that runtime
